...
> `compose33`의 실제 배포 구조는 한 AWS Ubuntu 서버에서 Frontend, Backend,
> PostgreSQL/pgvector, Redis를 함께 운영하고 Frontend와 Backend만 독립적으로 CI/CD하는
> 방식이다. 최초 서버 준비와 GitHub Actions 설정은 [DEPLOYMENT.md](DEPLOYMENT.md)를
> 기준으로 진행한다. 아래 내용은 복사 원본인 `compose22`의 분리 EC2 구조를 참고하기 위한
> 자료다.
