...
students = pd.DataFrame(  # 딕셔너리 데이터를 행과 열을 가진 DataFrame 표 구조로 변환합니다.
    [
...
avg_area, total_area = st.columns(2)

with avg_area:
    st.metric("평균 점수", f"{score_avg:.1f}점")  # 핵심 숫자나 상태값을 대시보드 지표 형태로 표시합니다.
with total_area:
    st.metric("총합", f"{score_sum}점")